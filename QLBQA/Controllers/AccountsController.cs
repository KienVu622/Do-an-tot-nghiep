...
namespace QLBQA.Controllers
{
    ...
    public class AccountsController : Controller
    {
        ...
        public ActionResult Login(LoginModel customer, string returnUrl = null)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        return RedirectToAction("BlockedNotif", "Accounts");
                    }
                    Session["CustomerId"] = khachhang.CustomerID.ToString();
                    ...
                    var claims = new List<Claim>
            {
                ...
                new Claim("CustomerId", khachhang.CustomerID.ToString())
                    ...
                    {
                        // Nếu có returnUrl, chuyển hướng đến returnUrl
                        return Redirect(returnUrl);
                    }
                    else
                    {
                        // Nếu không có returnUrl, chuyển hướng đến Dashboard
                        return RedirectToAction("Dashboard", "Accounts");
                    }
        ...
        public ActionResult ViewOrder(int? orderid)
        {
            ...
            {
                var ProductDetail = db.ProductDetails.Where(x => x.ProductDetailID == item.ProductID).FirstOrDefault();
